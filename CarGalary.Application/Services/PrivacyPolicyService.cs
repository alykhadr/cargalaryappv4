...
namespace CarGalary.Application.Services
{
    public class PrivacyPolicyService : IPrivacyPolicyService
    {
        ...
        public async Task<List<PrivacyPolicyResponseDto>> GetAvailableAsync()
        {
            var items = await _unitOfWork.PrivacyPolicies.GetAvailableAsync();
            return _mapper.Map<List<PrivacyPolicyResponseDto>>(items);
        }
        ...
        public async Task<PrivacyPolicyResponseDto?> GetLatestAvailableAsync()
        {
            var item = await _unitOfWork.PrivacyPolicies.GetLatestAvailableAsync();
            return item == null ? null : _mapper.Map<PrivacyPolicyResponseDto>(item);
        }
        ...
        public async Task UpdateAsync(int id, UpdatePrivacyPolicyRequestDto dto)
        {
            ...
            if (existing == null)
            {
                throw new Exception("PrivacyPolicy not found");
            }
