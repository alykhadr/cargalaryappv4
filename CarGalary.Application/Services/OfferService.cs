...
namespace CarGalary.Application.Services
{
    public class OfferService : IOfferService
    {
        ...
        public async Task<OfferResponseDto> CreateAsync(CreateOfferRequestDto dto){var e=_mapper.Map<Offer>(dto); NormalizeOfferFields(e); e.CreatedAt=DateTime.UtcNow; await _unitOfWork.Offers.CreateAsync(e); await _unitOfWork.SaveChangesAsync(); return _mapper.Map<OfferResponseDto>(e);} 
        public async Task UpdateAsync(int id, UpdateOfferRequestDto dto){var e=await _unitOfWork.Offers.GetByIdAsync(id); if(e==null) throw new Exception("Offer not found"); if(dto.IsAvailable==null) dto.IsAvailable=e.IsAvailable; _mapper.Map(dto,e); NormalizeOfferFields(e); await _unitOfWork.Offers.UpdateAsync(e); await _unitOfWork.SaveChangesAsync(); }
        ...
        private static void NormalizeOfferFields(Offer offer)
        {
            if (!offer.IsPercentage)
            {
                offer.PercentageValue = null;
                return;
            }

            if (offer.PercentageValue.HasValue)
            {
                offer.PercentageValue = decimal.Round(offer.PercentageValue.Value, 2, MidpointRounding.AwayFromZero);
            }
        }
    }
}
